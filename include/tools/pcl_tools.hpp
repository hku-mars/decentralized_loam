...
inline Eigen::Matrix< TT, 3, 1 > pcl_pt_to_eigen( const PointType &pt )
{
    return Eigen::Matrix< TT, 3, 1 >( ( TT ) pt.x, ( TT ) pt.y, ( TT ) pt.z );
}

template < typename T >
inline Eigen::Matrix< float, 3, 1 > pcl_pt_to_eigenf( const T &pt )
{
    return pcl_pt_to_eigen<float>(pt);
}

template < typename T >
inline Eigen::Matrix< double, 3, 1 > pcl_pt_to_eigend( const T &pt )
{
...
inline TT eigen_to_pcl_pt( const T &pt )
{
    TT res_pt;
    res_pt.x = pt( 0 );
    res_pt.y = pt( 1 );
...
inline pcl::PointCloud<PointType> eigen_pt_to_pcl_pointcloud( const vector<T> & eigen_pt_vec )
{
    pcl::PointCloud<PointType> pcl_pc_vec;
    size_t point_cloud_size = eigen_pt_vec.size();
    pcl_pc_vec.resize(point_cloud_size);
...
inline pcl::PointCloud< PointType > pointcloud_transfrom( pcl::PointCloud< PointType > pcl_pt_in, Eigen::Matrix< T, 3, 3 > tranfrom_R, Eigen::Matrix< T, 3, 1 > tranfrom_T )
{
...
inline int save_PCLXYZI_to_txt( const std::string &filename, const pcl::PointCloud<pcl::PointXYZI>::ConstPtr pcl_pt_vec, int if_append = 0 )
{
    std::fstream fos;
    if ( if_append > 0 )
    {
...
class PCL_point_cloud_to_pcd
{
  public:
    ...
    template <typename T>
    void save_to_pcd_files( string file_name, T cloud, int index = -1, int if_save_as_ply = 0 )
    {
        std::unique_lock<std::mutex> lock(m_mutex_save_files);
        char tempchar[ 5000 ];
        ...
        if ( index < 0 )
        {
            ...
            if ( if_save_as_ply )
            {
                sprintf( tempchar, "%s/%s_%d.ply", m_save_dir_name.c_str(), file_name.c_str(), m_save_files_index );
            }
            else
            {
        ...
        else
        {
            if ( if_save_as_ply )
            {
                sprintf( tempchar, "%s/%s_%d.ply", m_save_dir_name.c_str(), file_name.c_str(), index );
            }
            else
            {
        ...
        else
        {
            if ( IF_SAVE_ACSII )
            {
                pcl::io::savePLYFileASCII( tempchar, cloud );
            }
            else
            {
                pcl::io::savePLYFile( tempchar, cloud, 1 );
            }
        }
    };
...
template < typename T >
inline int load_from_pcd_file( const char *file_name, pcl::PointCloud< T > &target_cloud )
{
...
inline pcl::PointCloud< pcl::PointXYZRGBA > PCL_XYZI_to_RGBA( pcl::PointCloud< pcl::PointXYZI > &pt_in, float alpha = 0.1 )
{
    int                                  size = pt_in.size();
    float                                min_val = 3e8;
    float                                max_val = -3e8;
...
template < typename T, typename PointType >
inline vector< Eigen::Matrix< T, 3, 1 > > pcl_pts_to_eigen_pts( const typename pcl::PointCloud< PointType >::Ptr input_cloud )
{
